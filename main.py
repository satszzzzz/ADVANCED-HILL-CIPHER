...
def main(argv):
    ...
    block_size = 3
    try: 
        opts, _ = getopt.getopt(argv, "edcb", ["key=", "key2=", "image=", "block_size="])
    ...
    if(enciphering):
        ...
        enciphered_image,key2 = Hill.encipherImage(key, padded_image, block_size, '-e')
        enciphered_image.save(enciphered_filename, None)
        key.save(key_filename, None)
        key2.save(key2_filename, None)
        print("Success!")
        ...
        try: 
            ...
            key2_image=Image.open(key2_image)
        except Exception as e:
            print("Error reading key:", e)
            sys.exit()
        deciphered_image = Hill.decipherImage(
            key_image, input_image, block_size, complexKey, key2_image, '-d')
        deciphered_image.save("deciphered.jpg")
        print("Success!")


if __name__ == "__main__":
    x=['main.py','-d','--image=enciphered.tiff','--key=key.tiff','--key2=key2.tiff']
    y=['main.py','-e','--image=test_files/dog.jpg']
    main(x[1:])
